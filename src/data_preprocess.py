...
# Function to move 0 to the end for each row title
def move_zero_last(row):
    row_tensor = torch.tensor(row)

    sorted_row = torch.cat([row_tensor[row_tensor != 0], row_tensor[row_tensor == 0]])
    return sorted_row.tolist()
...
def parse_news(source, target, category2int_path, word2int_path, entity2int_path, mode):
    ...
    def parse_row(row):
        ...
        return pd.Series(new_row,index=[
    ...
    if mode == 'train':
        ...
        pd.DataFrame(category2int.items(), columns=['category', 'int']).to_csv(category2int_path,
                                                         sep='\t',
                                                         index=False)
        print(f'Please modify `num_categories` in `src/config.py` into 1 + {len(category2int)}')

        pd.DataFrame(word2int.items(), columns=['word', 'int']).to_csv(word2int_path,
                                                               sep='\t',
                                                               index=False)
        print(f'Please modify `num_words` in `src/config.py` into 1 + {len(word2int)}')

        pd.DataFrame(entity2int.items(), columns=['entity', 'int']).to_csv(entity2int_path,
                                                       sep='\t',
                                                       index=False)
        print(f'Please modify `num_entities` in `src/config.py` into 1 + {len(entity2int)}')
        ...
        # Apply operations to each row using the apply function
        parsed_news['title'] = parsed_news['title'].apply(move_zero_last)
        print(parsed_news)
